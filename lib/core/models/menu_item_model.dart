...
import 'package:gosharpsharp/core/models/item_file_model.dart';
...
class MenuItemModel {
  ...
  factory MenuItemModel.fromJson(Map<String, dynamic> json) {
    ...
    try {
      ...
      // If category parsing fails, we'll handle it below
    }

    // Safely parse restaurant with null check
    ...
    try {
      ...
      // If restaurant parsing fails, we'll handle it below
    }

    // Parse addon menus with proper null and error handling
    List<MenuItemModel>? parsedAddonMenus;
    if (addonMenusJson != null && addonMenusJson.isNotEmpty) {
      try {
        parsedAddonMenus = addonMenusJson
            .where((e) => e != null && e is Map<String, dynamic>)
            .map((e) {
          try {
            return MenuItemModel.fromJson(e as Map<String, dynamic>);
          } catch (error) {
            // Skip invalid addon items
            return null;
          }
        })
            .where((e) => e != null)
        ...
        if (parsedAddonMenus.isEmpty) {
          parsedAddonMenus = null;
        }
      } catch (e) {
        parsedAddonMenus = null;
      }
    ...
    return MenuItemModel(
      ...
      category:
      category ?? CategoryModel(id: 0, name: 'Unknown', description: 'N/A'),
      restaurant:
      restaurant ??
          RestaurantModel(
            id: 0,
      ...
      showOnCustomerApp: json['show_on_customer_app'] as bool?,
      files: filesJson
          .where((e) => e != null && e is Map<String, dynamic>)
          .map((e) {
        try {
          return ItemFileModel.fromJson(e as Map<String, dynamic>);
        } catch (error) {
          return null;
        }
      })
